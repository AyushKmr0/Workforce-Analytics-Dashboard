...
export function AdminTeamReports() {
  const [workDays, setWorkDays] = useState(1);
  const [departmentId, setDepartmentId] = useState('');
  const [selectedReport, setSelectedReport] = useState(null);
  const departments = useAsync(adminService.departments, []);
  const workReports = useAsync(
    () => adminService.workReports({ days: workDays, ...(departmentId ? { department_id: departmentId } : {}) }),
    [workDays, departmentId],
  );

  return (
    <div className="space-y-6">
      <div>
        <p className="text-sm font-bold uppercase tracking-wider text-blue-600">Work Reports</p>
        <h2 className="text-3xl font-black tracking-tight text-slate-950">Company Work Reports</h2>
      </div>
      <section className="card p-5">
        ...
        <p className="mt-3 text-sm text-slate-500">{workReports.data?.summary?.start_date || '-'} to {workReports.data?.summary?.end_date || '-'}</p>
            ...
            <tbody className="divide-y divide-slate-100">
              {(workReports.data?.items || []).map((report) => (
                <tr key={report.id} className="cursor-pointer transition hover:bg-blue-50/50" onClick={() => setSelectedReport(report)}>
                  ...
                  <td className="px-4 py-3">{report.employee?.department?.name || 'Unassigned'}</td>
              ...
              {!(workReports.data?.items || []).length && <tr><td className="px-4 py-6 text-center text-slate-500" colSpan="6">No work reports found.</td></tr>}
            </tbody>
          </table>
        </div>
      </section>
